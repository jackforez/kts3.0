...
import { key, mail, mapPin, pencil, phone, userName } from "../ultis/svgs";
...
const EditPartner = () => {
  ...
  useEffect(() => {
    document.title = "Thay đổi thông tin đối tác - KTSCORP.VN";
    dispatch(setCurrentPage("Thay đổi thông tin đối tác"));
  }, []);
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        const res = await ktsRequest.get(`/users/find/${id}`, {
          headers: {
        ...
        setInputs(res.data);
        setUser(res.data);
        if (res.data.parentUser !== currentUser._id) {
  ...
  const handleChangeInfo = async () => {
    ...
    try {
      const res = await ktsRequest.put(
        `users/${user._id}`,
        { ...inputs, img: url, updatedBy: currentUser._id },
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
  ...
  const handleChangePwd = async () => {
    ...
    try {
      ...
      toast.success("Thay đổi mật khẩu đối tác thành công");
      setCheckChangePwd(false);
      dispatch(loaded());
    } catch (error) {
      toast.error(error.response ? error.response.data : "Network Error!");
      setCheckChangePwd(false);
        ...
        <div className="md:w-1/4 w-full md:py-12 py-3 px-2 flex flex-col items-center">
          <div className="w-32 h-32 aspect-square rounded-full relative flex justify-center items-center bg-orange-500 text-white">
            {inputs.img || file ? (
              ...
              textAvatar(inputs?.displayName || "ktscorp.vn")
            )}
            <button
          ...
          <div className="md:w-1/2 w-full space-y-3">
            ...
            <div className="w-full">
              <Button
                ...
                loading={loading && inputs !== user}
                disabledBy={inputs === user || loading}
                animation={true}
                padding={"sm"}
              >
                Cập nhật thông tin
